...
clean: ## remove generated byte code, coverage reports, and build artifacts
	...
	find poll_remake/translations -name djangojs.mo -exec rm -f {} +
	find poll_remake/translations -name djangojs.po -exec rm -f {} +
	find poll_remake/translations -name textjs.mo -exec rm -f {} +
...
quality: ## check coding style with pycodestyle and pylint
	pycodestyle poll_remake --max-line-length=120
	pylint poll_remake
...
test: ## run tests in the current virtualenv
	...
	python run_tests.py --with-coverage --cover-package=poll_remake
...
extract_translations: ## extract strings to be translated, outputting .po files
	...
	ls poll_remake/public/handlebars/*.handlebars \
